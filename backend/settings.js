/* global accessally_wp_courseware_convert_object */
...
jQuery(document).ready(function ($) {
	...
	var $wait_overlay = $('#accessally-wp-courseware-convert-wait-overlay');
	$(document).on('click touchend', '[accessally-convert-course]', function() {
		var conf = confirm('Do you want to convert all items in this WP Courseware course to regular WordPress pages?')
		if(conf !== true){
			return false;
		}
		$wait_overlay.show();
		var course_id = $(this).attr('accessally-convert-course'),
			conversion_operation = $('#accessally-wp-courseware-operation-' + course_id).val(),
			data = {
				action: 'accessally_wp_courseware_convert',
				id: course_id,
				op: conversion_operation,
				nonce: accessally_wp_courseware_convert_object.nonce
			};

		$.ajax({
			type: "POST",
			url: accessally_wp_courseware_convert_object.ajax_url,
			data: data,
	...
	function process_convert_result(response) {
		...
		if ('status' in result) {
			...
			var $display_elem = $('#accessally-wp-courseware-convert-container');
	...
	$(document).on('click touchend', '[accessally-revert-course]', function() {
		var conf = confirm('Do you want to change previous converted pages back to a WP Courseware Course?'),
		...
		$wait_overlay.show();
		var data = {
				action: 'accessally_wp_courseware_revert',
				id: course_id,
				nonce: accessally_wp_courseware_convert_object.nonce
			};
